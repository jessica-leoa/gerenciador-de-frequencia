...
def form():
    if request.method == "POST":
        ...
        # Validação 2: Carga Horária (Positiva e Max 400)
        try:
            carga_horaria = int(carga_horaria)
            if carga_horaria <= 0 or carga_horaria > 400:
                flash("Carga Horária deve ser positiva e no máximo 400 horas.", "danger")
                return render_template("turma_form.html")
        except ValueError:
...
def editar_turma(id):
    ...
    try:
        carga_horaria = int(carga_horaria)
        if carga_horaria <= 0 or carga_horaria > 400:
            flash("Carga Horária deve ser positiva e no máximo 40 horas.", "danger")
            return redirect(url_for('turmas.list_all'))
    except ValueError:
        flash("Carga Horária deve ser um número inteiro válido.", "danger")
        return redirect(url_for('turmas.list_all'))
